...
function AddTaskModal({ isOpen, onRequestClose, tasks, onFullReset, setTaskAdding }) {
  ...
  const Confirm = () => {
    ...
    if(add){
      ...
      if (tasksToAdd.length>1) {
        ...
        setMessage(t("addNewTasks.buildingNumbersSelectionRequired"));
      }
  ...
  return (
    ...
    >
      <>
        <h2 className="text-2xl font-bold mb-4">{t("addNewTasks.addNewTasks")}</h2>
        <p className="text-gray-700 mb-4">{t("addNewTasks.chooseBuildingNumberToAddTasks")}</p>
        <p className='mb-2'>{t("addNewTasks.selected")}: {tasksToAdd.length>0&& tasksToAdd[0]}-{tasksToAdd.length>0&& tasksToAdd[tasksToAdd.length-1]}</p>
        <p className='mb-2 text-red-600'>{message}</p>
        <div className="flex flex-col mb-2">
        <input
          className={`p-3 rounded bg-gray-200 w-full ${inputError ? 'border-red-500' : ''}`}
          type="text"
          placeholder={t("addNewTasks.writeBuildingLetter")}
          ...
          maxLength={4}
        />
        {inputError && <p className="text-red-500">{t("addNewTasks.enterOnlyLetters")}</p>}
        </div>
        <div className="flex mb-4">
          {/* Select for A-Z letters */}
          <div className='flex flex-col w-1/2'>
          <label htmlFor="letter" className='text-gray-400'>{t("addNewTasks.from")}</label>
          ...
          <div className='flex flex-col w-1/2'>
          <label htmlFor="letter" className='ml-3 text-gray-400'>{t("addNewTasks.to")}</label>
